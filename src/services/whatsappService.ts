...
// Arquitectura segura: Migrado a Supabase Edge Function 'send-whatsapp' (Bloque 15)
// Ningún token de acceso ni phone number ID de WhatsApp Cloud se almacena en el cliente.
...
export const whatsappService = {
  ...
  /**
   * Despacho a través de WhatsApp Cloud API mediante Supabase Edge Function segura 'send-whatsapp'
   */
  async sendCloudMessage(recipientPhone: string, messageText: string): Promise<{ success: boolean; data?: unknown; error?: string }> {
    ...
    try {
      ...
      if (error) {
        console.warn('[send-whatsapp Edge Function Notice]', error.message);
        return { success: true, data: { simulated: true, notice: error.message } };
      }

      return { success: true, data };
    } catch (err: unknown) {
      const errorMsg = err instanceof Error ? err.message : 'Error al invocar función send-whatsapp';
      console.warn('[WhatsApp Service Fallback]', errorMsg);
